...
class PageDataView(QFrame):
        ...
        def normalize_data(self):
            ...
            for point in self.waveformData:
                if point.y() > maxY:
                    maxY = point.y()
            for point in self.waveformData:
                self.normalizedData.append(QPointF(point.x() / maxX * self.width(), self.height() - (point.y() / maxY * self.height())))
    ...
    def __init__(self, parent):
        ...
        # self.waveform_widget.waveformData = generate_random_points()
        self.waveform_widget.waveformData = generate_random_points1()
        self.waveform_widget.show()
        # self.waveform_widget.saveImage("waveform.png")
